...
function set_feature_blog(elm)
  {
    ...
    (new Request.JSON({
      'format': 'json',
      'url' : '<?php echo $this->url(array('module' => 'blog', 'controller' => 'admin-manage', 'action' => 'featured'), 'default', true) ?>',
      'data' : {
        'format' : 'json',
        'blog_id' : blog_id,
  ...
  <tbody>
    <?php foreach ($this->paginator as $item): ?>
      <tr>
        ...
        <td>
            <?php echo @isset($this->cateArr[$item->category_id]) ? $this->cateArr[$item->category_id] : $this->translate('no category') ?>
        </td>
        <td><?php echo $item->getOwner()->getTitle() ?></td>
        <td><?php echo $this->locale()->toNumber($item->view_count) ?></td>
        <td><?php echo $this->locale()->toDateTime($item->creation_date) ?></td>
        <td>
            <input type="checkbox" name="<?php echo $item->blog_id;?>" onclick="set_feature_blog(this);" <?php echo ($item->featured) ? "checked='true'" : "";?> value="<?php echo ($item->featured) ? "1" : "0";?>" />
          </td>
        <td>
