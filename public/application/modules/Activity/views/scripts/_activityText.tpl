              ...
              <?php foreach( $action->getAttachments() as $attachment ): ?>
                ...
                <?php elseif( $attachment->meta->mode == 1 ): // Thumb/text/title type actions ?>
                  <div>
                    ...
                    ?>
                    <?php if ( ($attachment->item instanceof Album_Model_Photo) || ($attachment->item instanceof Blog_Model_Blog) ):?>
                    
	                    <?php if( $attachment->item->getPhotoUrl() ): ?>
	                      <?php echo $this->htmlLink($attachment->item->getHref(), $this->itemPhoto($attachment->item, 'thumb.main', $attachment->item->getTitle(), array('style' => 'max-width: 100%; max-height: 320px;')), $attribs) ?>
	                    <?php endif; ?>
	                    
	                <?php elseif ($attachment->item instanceof Event_Model_Event) : ?>
	                
	                	<?php if( $attachment->item->getPhotoUrl() ): ?>
	                      <?php echo $this->htmlLink($attachment->item->getHref(), $this->itemPhoto($attachment->item, 'thumb.main', $attachment->item->getTitle(), array('style' => 'width: 100%;')), $attribs) ?>
	                    <?php endif; ?>
