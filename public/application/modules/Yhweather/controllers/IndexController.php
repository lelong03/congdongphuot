...
class Yhweather_IndexController extends Core_Controller_Action_Standard
{
	...
	public function editLocationAction()
	{
		...
		if (!$viewer || $viewer->getIdentity() == 0) {
			...
			$this->view->message = $this->view->translate('yhweather_Please login to continue.');
			return;
		}

		if (!$object_id || !$object_type) {
			$this->view->error = true;
			$this->view->message = $this->view->translate('yhweather_Please type your location.');
			return;
		}
		...
		if (!$this->view->can_edit_location) {
			$this->view->error = true;
			$this->view->message = $this->view->translate('yhweather_You cannot edit this location.');
			return;
		}

		$locationTbl = Engine_Api::_()->getDbTable('locations', 'yhweather');
