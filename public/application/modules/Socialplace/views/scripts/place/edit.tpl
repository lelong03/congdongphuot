...
<?php
    $phone = "[]";
    if (is_array($this -> place -> phone) && count($this -> place -> phone))
    {
        $phone = Zend_Json::encode($this -> place -> phone);
    }
    elseif (!is_array($this -> place -> phone))
    {
...
window.addEvent('domready', function() {
	$('add_more_phone').addEvent('click', function() {
  		addMoreAddPhone('');
  	});


	phones = <?php echo $phone ?>;
	if (phones.length > 1)
	{
		for(i = 1; i < phones.length; i++)
		{
			addMoreAddPhone(phones[i]);
		}
	}
	
  	function addMoreAddPhone(value) {
        ...
        newAddInfo.grab(remove);
        newAddInfo.inject('add_more_phone-wrapper', 'before');
        return;
    }	
});
</script>
