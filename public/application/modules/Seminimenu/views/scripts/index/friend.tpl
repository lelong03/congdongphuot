...
<?php foreach ($this->freqs as $item):?>
    ...
    <li id="freqs_<?php echo $subject->getIdentity()?>">
		...
		<div class="seminimenu_ContentlistInfo">
    		...
    		<div class="seminimenu_MessDescription" <?php if($totalFriends > 0):?> title = "<?php echo $str_mutural_users; ?>" <?php endif;?>> 
    			<?php echo $this->translate(array("%s mutual friend","%s mutual friends",$totalFriends), $this->locale()->toNumber($totalFriends));?>
    		</div>
			<div class="seminimenu_Friendsbutton" id="friend_action_<?php echo $subject->getIdentity()?>"> 
				<button class="seminimenu_FriendsbuttonConfirm" onclick="return doConfirm(<?php echo $subject->getIdentity()?>)"> <?php echo $this->translate("Confirm") ?> </button>
				<button class="seminimenu_FriendsbuttonCancel" onclick="return doCancel(<?php echo $subject->getIdentity()?>)"> <?php echo $this->translate("Not Now") ?> </button>
			</div>
		</div>
    </li>
<?php endforeach;?>
...
var doConfirm = function(rid)
  {
      ...
      var img_loading = '<?php echo $this->baseUrl(); ?>/application/modules/Seminimenu/externals/images/loading_friend.gif';
      $('friend_action_'+rid).innerHTML = '<center><img src="'+ img_loading +'" border="0" /></center>';
      new Request.HTML({
           'url'    :    en4.core.baseUrl + 'seminimenu/index/confirm-friend/',
           'data' : {
                'format' : 'html',
                'resource_id' : rid
            },
  ...
  var doCancel = function(rid)
  {
      ...
      $('friend_action_'+rid).innerHTML = '<center><img src="'+ img_loading +'" border="0" /></center>';
      new Request.HTML({
           'url'    :    en4.core.baseUrl + 'seminimenu/index/cancel-friend/',
           'data' : {
                'format' : 'html',
                'resource_id' : rid
            },
