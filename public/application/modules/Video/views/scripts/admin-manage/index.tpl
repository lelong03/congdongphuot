...
function selectAll()
{
 ...
 function killProcess(video_id) {
  ...
  function set_feature_video(elm)
  {
    video_id = elm.name;
    if (elm.value == "1")
      featured_value = 0;
    ...
    (new Request.JSON({
      'format': 'json',
      'url' : '<?php echo $this->url(array('module' => 'video', 'controller' => 'admin-manage', 'action' => 'featured'), 'default', true) ?>',
      'data' : {
        'format' : 'json',
        'video_id' : video_id,
    ...
    <tbody>
      <?php foreach ($this->paginator as $item): ?>
        <tr>
          ...
          <td>
            <input type="checkbox" name="<?php echo $item->video_id;?>" onclick="set_feature_video(this);" <?php echo ($item->featured) ? "checked='true'" : "";?> value="<?php echo ($item->featured) ? "1" : "0";?>" />
          </td>
          <td>
